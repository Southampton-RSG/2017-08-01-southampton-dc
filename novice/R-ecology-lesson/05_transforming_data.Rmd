...
there is evidence that kangeroo rats compete directly with four other rodent 
species, generating a similar figure to the one below, to confirm their observations.

Concretely, we are going to look at the effect of kangeroo rat exlusion on the 
populations of smaller granivores in two 0.25 hectare plots versus two plots
where kangeroo rats were allowed access.

This figure shows the number of captures of threes species of kangeroo Rats
...
resources and Panel B shows the effect of excluding the kangeroo Rats.
...
Kangeroo Rat species with the five smaller granivores, so it would be useful
to have a variable the catergorised the rodents as either "Kangeroo Rat" or
"Granivore".
...
interestd in. Here they are called: `rodents` and `krats`.
...
[here](https://github.com/weecology/PortalData/blob/master/Rodents/Portal_rodent_species.csv), but to save time the relevent codes are shown below.
...
`na.rm = TRUE` arguement is given to remove missing values from the data, 
...
In the previous section we used this pairing to check when the speices were 
correctly assigned their rodent type. We first grouped `species_id` and 
`rodent_type` together and then used `summarise()` without any arguements to
...
*Q: Is there evidence that kangeroo rats compete directly with four other 
...
a bit more. Concretely, we'd like to summarise the kangeroo rat and granivore
populations on a monthly or quarterly basis. Or by genus.

This is where grouped summaries are very useful. 

Grouping simply means grouping the variables of interest that we want to operate on, here they are the `rodent_type` (kangeroo rat and granivore), 
the `quarter` (the time period) and the `plot_type` (kangeroo rat exclusion or
control). The summary we would like is the number of captures for each group e.g.
number of captures of kangeroo rats for the first quarter of 1980 in the control 
...
months worth of captures, so to get the quartely average number of captures we
...
We have now transformed the dataset from a 35,000 by 13 table to the 185 by 4 
table that we need for re-creating the plot shown in the paper.
