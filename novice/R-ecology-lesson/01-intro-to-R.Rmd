...
syntax, it is good practice to use always `<-` for assignments.
...
> <kbd>Crtl</kbd> + <kbd>Shift</kbd> + <kbd>C</kbd>.
...
> ### Call out
>
> What is called `objects` in `R` is known as `variables` in many other programming languages.
> Depending on the context `object` and `variable` can have drastically different meanings.
> However, in this lesson the two words are used synonymously.
> For more information see: https://cran.r-project.org/doc/manuals/r-release/R-lang.html#Objects
...
will assume there is a variable called mouse, rat, etc.
...
Here, `<` stands for "smaller than", `>` or "bigger than", `>=` for "bigger or
equal to", and `==` for "equal to". The double equal sign `==` should not be
confused with the single `=` sign, which assigns to a variable (roughly speaking
the same as "`<-`").

A common task is to search for certain strings in a vector. One could use the "or"
operator `|` but this can become quickly tedious. The function `%in%` allows you to test
