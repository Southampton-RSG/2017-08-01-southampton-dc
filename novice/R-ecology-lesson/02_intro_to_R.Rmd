...
>
> * Used ggplot2 to create visualisations to answer questions about two datasets.
> * Performed object assignments and functions calls.
> * Been introduced to the six atomic vector types in R.
> * Grouped data types into a `list` and a `data.frame` and learnt how to access
> individual elements of these objects. The learner will experience how R 
> stores data in the R environment, and how to interact with it.
>
...
We should have the tidyverse installed and if it isn't loaded already, we load it using the `library()` function. We pass
the name of the package `tidyverse` to the function by putting
it inside the round brackets: `library(tidyverse)` and pressing
`Enter`. 

As part of the tidyverse package, `ggplot2` is then loaded into 
our R environment and we're ready to go.
...
```{r, mpg_city,echo=FALSE, purl=FALSE, eval = FALSE}
## Answers
...
Object name style is a matter of choice, but must start with a 
letter and can only contain letters, numbers, `_` and `.`. We 
recommend using descriptive names and using  the underscore `_`
between words. Some special symbols cannot be used in variable 
...
Read the following code as *"make an object called my_sequence that stores a sequence of numbers **from** 2 **to** 20 **by**
intervals of 2*.
