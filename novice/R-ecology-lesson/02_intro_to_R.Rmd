...
a one dimensional group of numbers, in a sequence from one to ten.

We're not going to be working directly with atomic vectors in this workshop, 
...
+ Integers: whole numbers, specified with an uppercase L, e.g. `int <- 2L`
...
Factors are Rs way of storing catergorical information such as eye colour or
car type. A factor is something that can only have certain values.

Factors are useful as they code variables such as "red" or "blue" to integer 
...
it's often not what we want as a defualt. Fortunately, in the tidyverse strings
are not treated as factors by defualt.
...
vector of numbers and element two is a vector ofa single character.
...
dimensions. We won't be using these either, but they are valid R objects.
...
We'rehere are many places to start, but swirl can teach you interactively, and at your own pace in RStudio.

Just follow the instructions via this link: http://swirlstats.com/students.html

Hands-On Programming with R by Garrett Grolemund is another great resource for
learning R.
