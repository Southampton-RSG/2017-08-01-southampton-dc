...
But this isn't a count of the captures, it's how many times each genus appears
in a row of `by_month_genus`. Remember how we summarised this data by captures
per month.

Often when people talk about bar charts they are referring to when the height
of the bar is present in the data, here that would be the `captures` column.
...
To learn more about statistical transformations have a look at the [ggplot2 cheatsheet](https://www.rstudio.com/wp-content/uploads/2016/11/ggplot2-cheatsheet-2.1.pdf).
...
Alternatively we could set `postion = dodge` and we get the third plot, where
the bars for each genus are side by side, again allowing us to see both the
proportions and the mean values.

```{r,purl=FALSE,echo=TRUE}
# Create a stacked bar chart of plot type coloured by 
...
Recall that factors are how R represents catergorical variables, variables with
a limited number of values, such as genus.

Also recall *factors look like strings, but behave like integers*. That is to 
say they are words that have values called levels, and an order.
...
explore the power of factors, but here we're going to use the `fct_reorder()` 
function to convert `genus` to a factor and in doing so set the level according 
to to the number of captures. 

Note we still keep x and y mappings, but now x is now a function of both the 
genus and the capture columns.

This gives us a nice ordered bar chart.
...
> ### Challenge
> To illustrate using the size aesthetic, recreate the `by_month_mean` plot using
geom_point() with the additional arguement of `size = rodent_type`.
>
> Also use `fct_reorder()` to change the x mapping to a factor that is a function
...
a general case. However, the underlying form remains the same.
...
To this we add all the additional functions and arguements that will change
the plot to make it exactly how we want. It takes time, but remember code is 
resusable. Solve it once and you've solved it forever.
...
Recall our boxplot with points overlaid. Following the facet function, 
...
out the [cowplot](https://cran.r-project.org/web/packages/cowplot/vignettes/introduction.html) pacakge that works with ggplot2. This enables easy combinations
