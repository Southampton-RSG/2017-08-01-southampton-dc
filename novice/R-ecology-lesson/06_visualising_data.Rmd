...
argument is `~` followed by the variable name, here  `rodent_type`. This argument is called a formula. There are many additional arguments that can
...
> ### Challenge
> Try plotting `by_quarter` using `facet_grid()` and arguments 
...
say they are strings of characters with associated values called levels that 
indicate an order.
...
It's unlikely that you'll need to statistical transformations, positional adjustments, coordinate adjustments, and facets all in the same plot, but this 
